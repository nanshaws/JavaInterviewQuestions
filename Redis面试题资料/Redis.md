...
public class CacheUtils{
    ...
    public CacheData getCacheData(String key){
        ...
        synchronized(CacheUtils.class){
            ...
            CacheData cacheData = this.cacheDataDao.query(key);
    ...
    public static CacheUtils getCacheUtils(){
        if(ObjectUtils.isNull(cacheUtils)){
           synchronized(CacheUtils.class){
                if(ObjectUtils.isNull(cacheUtils)){
...
> IO多路复用是 IO 模型的一种，有时也称为异步非阻塞 IO，是基于经典的 Reactor 设计模式设计的。多路指的是多个 Socket 连接，复用指的是复用一个线程。多路复用主要有三种技术：Select，Poll，Epoll。
>
